...
RUN apt-get update -y && apt-get upgrade -y && apt-get install -y cmake ninja-build valgrind git pkg-config doxygen qemu-user curl unzip gcc-9 g++-9 clang-10 clang-tidy-10 clang-tools-10 gcc-powerpc-linux-gnu gcc-powerpc64-linux-gnu gcc-arm-linux-gnueabihf gcc-9-aarch64-linux-gnu gcc-9-riscv64-linux-gnu
